...
public class MonticuloSVG implements EstructuraGrafica{

	/* La coleccion a representar como un arreglo */
	public MonticuloMinimo<ValorIndexable<Integer>> coleccion;

	/* Constructor */
	public MonticuloSVG(MonticuloMinimo<ValorIndexable<Integer>> c) {
		coleccion = c;
	}
	...
	public void imprime() {
		...
		for (int i = 0; i < coleccion.getElementos(); i++ ) {
			...
			res = res + "\t<text fill='black' font-family='sans-serif' font-size='15' x='" + (x + 25) + "' y='37' text-anchor='middle'>" + coleccion.get(i).getElemento() + "</text>\n";
